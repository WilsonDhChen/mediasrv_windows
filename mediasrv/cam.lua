...
config.onvif.profilefromplugin(1)
...
function OnStreamPlayStarted(  ctx )
	...
	NotifyConnections(  string.upper(stream.getdevid(ctx)), stream.getstream(ctx),stream.getsrccid(ctx),stream.getapp(ctx),stream.getconnections(ctx) )
	return 0 ;
end
------------------------------------------------------
-----播放关闭通知
...
function PTZStop( app , streamid ,bPanTilt , bZoom )


end
-----------------
function PTZRelativeMove(app, streamid, zoom_x, x, y)


end
-----------------
function PTZAbsoluteMove(app, streamid, zoom_x, x, y)


end
-----------------
function PTZContinuousMove(app, streamid, zoom_x, x, y)
	if ( x < 0 )
	then								
		rdeu.sendstr(udpSoc,"event=dsend;id="..streamid..";value=CAM_LEFT",notifyAddr,notifyPort)
	elseif( x> 0 )
	then
		rdeu.sendstr(udpSoc,"event=dsend;id="..streamid..";value=CAM_RIGHT",notifyAddr,notifyPort)
	end


	if ( zoom_x < 0 )
	then								
		rdeu.sendstr(udpSoc,"event=dsend;id="..streamid..";value=CAM_LONG",notifyAddr,notifyPort)
	elseif( zoom_x> 0 )
	then
		rdeu.sendstr(udpSoc,"event=dsend;id="..streamid..";value=CAM_SHORT",notifyAddr,notifyPort)
	end

end

-----------------
function PTZContinuousMoveWithTimeout(app, streamid, timeout, zoom_x, x, y)

end


function GetOnvifProfile(arry,app , streamid)
	onvif.addprofile(arry,streamid)
end

function GetOnvifProfiles(arry)
	
	local cur,errorString = exesql( conn, "SELECT sId FROM devices WHERE nOnlineFlag=1"      )
	local row = cur:fetch({},"a")
	
	while row do
	
	local sid = string.lower(row.sId);
	--print(sid)
	onvif.addprofile(arry,sid)
...
function OnGetSourceStreamURL(app , streamid, par, protocol)
	print("Send TRANS_VIDEO to "..streamid)
	rdeu.sendstr(udpSoc,"event=dsend;id="..streamid..";value=TRANS_VIDEO",notifyAddr,notifyPort)
end
